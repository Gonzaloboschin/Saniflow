  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-border p-5">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-[#4B5B54] mt-2">
          ...
          {cliente.estado === "en_riesgo" && (
            <span className="flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full bg-[#FBEAE3] text-warn">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <Kpi icon={DollarSign} label="Total facturado" value={fmtMoney(cliente.total_facturado)} accent="#0F5C56" />
        <Kpi icon={AlertTriangle} label="Reclamos" value={cliente.total_reclamos} accent={cliente.total_reclamos > 0 ? "#C2542B" : "#16241F"} />
      ...
      {problemas && problemas.length > 0 && (
        <div className="bg-white rounded-lg border border-border p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {problemas.map((p) => (
              <span key={p.etiqueta} className="text-xs font-semibold px-2.5 py-1 rounded-full bg-[#FBEAE3] text-warn">
      ...
      <div className="bg-white rounded-lg border border-border p-4">
        ...
        {!historial || historial.length === 0 ? (
          ...
          <div className="space-y-2">
            {historial.map((t) => (
              <div key={t.id} className="flex items-center justify-between gap-3 py-2 border-t border-[#EEF2F0] first:border-t-0 first:pt-0">
      ...
      <div className="bg-white rounded-lg border border-border p-4">
        ...
        {!interacciones || interacciones.length === 0 ? (
          ...
          <div className="space-y-3">
            {interacciones.map((i) => (
              <div key={i.id} className="py-2 border-t border-[#EEF2F0] first:border-t-0 first:pt-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <span
                    className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${
                      i.tipo === "reclamo" ? "bg-[#FBEAE3] text-warn" : "bg-[#0F5C5618] text-primary"
                    }`}
                  >
                ...
                {i.descripcion && <p className="text-sm text-[#4B5B54] mt-0.5">{i.descripcion}</p>}
